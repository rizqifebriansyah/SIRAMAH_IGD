...
class VKController extends Controller
{
    ...
    public function formermvk(Request $request)
    {
        ...
        $bidanpoli = DB::select('SELECT fc_nama_unit1(a.kode_unit) AS nama_unit, a.* FROM erm_hasil_assesmen_keperawatan_rajal a WHERE a.kode_unit IN ("1012", "1032") AND a.STATUS = "1" AND a.no_rm = ?', [$norm]);
        $dokpoli = DB::select('SELECT 
            a.nama_dokter
            , a.`id_kunjungan`
            , a.`id_pasien`
            , a.`tgl_entry`
            ...
            , a.`frekuensi_nafas`
            , a.`suhu_tubuh`
            , a.`riwayat_kehamilan_pasien_wanita`
            , a.`diagnosakerja`
            , a.`diagnosabanding`
            , a.`rencanakerja`
            , a.`pemeriksaan_fisik`
            , a.`tindak_lanjut`
            , a.`keterangan_tindak_lanjut`
            , a.`keterangan_tindak_lanjut_2`
            , fc_nama_unit1(a.kode_unit) AS nama_unit
            FROM assesmen_dokters a
            WHERE a.kode_unit IN ("1012", "1032")
            AND a.STATUS = "1" AND a.id_pasien = ?', [$norm]);
    ...
    public function updateassesvk(Request $request)
    {
        ...
        try {
            ...
            if ($cekcpp[0]->status == 1 && $cekcppl[0]->status == 1 && $cekcpp[0]->kode_unit == '3005' && $cekcppl[0]->kode_unit == '3005') {
                ...
                $cekcppl = DB::select('UPDATE erm_cppt_kebidanan_lanjutan SET status = "3"  WHERE no_rm = ? AND kode_kunjungan = ? AND kode_unit = "3005"', [$norm, $kj]);
